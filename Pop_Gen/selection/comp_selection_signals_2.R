...
# Please note this custom code has been cleaned using Claude
################################################################################
...
# Load watchlist
watchlist <- read.table(watchlist_file)
...
xp_split    <- unique(trimws(unlist(strsplit(xp_regions, "\\|"))))
...
for (region in all_regions) {
  ...
  # --------------------------------------------------------------------------
  # STEP 4: Fill coordinates and find overlapping genes
  # --------------------------------------------------------------------------

  cat("\nStep 4: Finding genes in selected regions...\n")

  # Use coalesce to fill missing start/end coordinates in priority order:
  # TD > iHS > IBD > XPEHH
  df_region <- df_region %>%
    mutate(
      TD_start_filled = coalesce(TD_start, iHS_start, IBD_start, XPEHH_start),
      TD_end_filled   = coalesce(TD_end,   iHS_end,   IBD_end,   XPEHH_end)
    )

  regions <- df_region %>%
    select(Chr, TD_start_filled, TD_end_filled,
           TD_present, iHS_present, IBD_present, XPEHH_present,
           n_support, evidence_strength) %>%
    rename(TD_start = TD_start_filled, TD_end = TD_end_filled) %>%
    filter(!is.na(TD_start))
  ...
  if (length(matching_genes) > 0) {
    ...
    matching_genes_with_evidence <- overlapping[
      gene_ID %in% matching_genes |
        sapply(gene_ID, function(gid) any(grepl(paste(matching_genes, collapse = "|"), gid)))
    ][, .(
      max_evidence_strength = evidence_strength[which.max(n_support)],
      max_n_support         = max(n_support)
    ), by = gene_ID]

    matching_genes_summary <- paste(
      paste0(matching_genes_with_evidence$gene_ID,
  ...
  fst_file <- file.path(fst_dir, paste0(gsub(" ", "_", region), "_vs_all.single.snp_fst_df.csv"))
  ...
  # Filter for high Fst using the user-defined threshold
  fst <- fst_data[Fst >= fst_threshold]
  fst[, CHR := gsub("Pf3D7_0", "", CHR)]
  fst[, CHR := gsub("Pf3D7_",  "", CHR)]
  fst[, CHR := gsub("_v3",     "", CHR)]
  fst[, CHR := as.character(CHR)]

  cat(sprintf("  Total high Fst SNPs (>= %.1f): %d\n", fst_threshold, nrow(fst)))

  # Initialise snp_near_TD as empty in case foverlaps returns nothing
  snp_near_TD <- data.table()
  ...
  if (nrow(fst) == 0) {
    ...
    if (nrow(snp_near_TD) > 0) {
      ...
      fwrite(snp_near_TD,
             file.path(region_dir, paste0(gsub(" ", "_", region), "_high_fst_snps_detailed.tsv")),
             sep = "\t")
      fwrite(snp_summary,
             file.path(region_dir, paste0(gsub(" ", "_", region), "_high_fst_snps_by_gene.tsv")),
             sep = "\t")
      cat("  Results saved.\n")
    }
  }

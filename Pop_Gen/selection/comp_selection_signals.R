...
tajima_dir      <- "path/to/tajima_directory"       # Directory containing .Tajima.D files
tajima_suffix   <- "_Pf_Nov_24.txt.genotyped.vcf.gz_tajima.txt.Tajima.D"  # Suffix to strip from filenames
ihs_file        <- "path/to/cr_ihs_all_categories_annot.tsv"
xpehh_file      <- "path/to/cr_xpehh_all_categories_annot.tsv"
ibd_file        <- "path/to/hmmIBD_ibd_win10kb.tsv"
exclude_regions <- "Horn"                           # Region pattern to exclude (set NULL to keep all)
chromosomes_exclude <- c("Pf_M76611", "Pf3D7_API_v3")  # Chromosomes to exclude
...
list_files <- list.files(path = tajima_dir, pattern = ".Tajima.D", full.names = TRUE)
list_f <- list()
for (file in list_files) {
  list_f[[basename(file)]] <- read.csv(file, sep = "\t")
}

tajima_d <- rbindlist(list_f, idcol = TRUE)
tajima_d$Region <- gsub(tajima_suffix, "", tajima_d$.id)
tajima_d$Region <- gsub("_", " ", tajima_d$Region)
tajima_d <- tajima_d %>% filter(!CHROM %in% chromosomes_exclude)
...
cr_ihs   <- read.delim(ihs_file)
cr_xpehh <- read.delim(xpehh_file)

if (!is.null(exclude_regions)) {
  cr_xpehh <- cr_xpehh[!grepl(exclude_regions, cr_xpehh$category_name), ]
  cr_ihs   <- cr_ihs[!grepl(exclude_regions, cr_ihs$category_name), ]
}
...
# Helper to clean chromosome names
...
setDT(TD); setDT(cr_ihs); setDT(ibd); setDT(cr_xpehh)
...
all_chrs <- sort(unique(na.omit(c(TD$Chr, cr_ihs$Chr, ibd$Chr, cr_xpehh$Chr))))
